    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                self.available = True
                print("✅ Groq LLM initialisé (Llama 3.3 70B)", file=sys.stderr)
            except:
                try:
                    self.client.chat.completions.create(
                    ...
                    self.available = True
                    print("✅ Groq LLM initialisé (Llama 3.1 8B)", file=sys.stderr)
                except Exception as e:
                    print(f"⚠️  Groq LLM non disponible: {e}", file=sys.stderr)
        
        except ImportError:
            print("⚠️  Package 'groq' non installé (pip install groq)", file=sys.stderr)
...
{{
  "column_name": {{
    ...
    "reason": "..."
        ...
        try:
            print("🤖 Consultation Groq LLM pour détection des types...", file=sys.stderr)
            ...
            print(f"✅ Types détectés pour {len(result)} colonnes", file=sys.stderr)
            return result
...
def clean_basic(df, row_thresh=0.5, col_thresh=0.3):
    ...
    print("\n📋 Nettoyage de base...", file=sys.stderr)
...
def clean_dates(df, decisions):
    ...
    for col, decision in decisions.items():
        ...
        if decision.get('is_date'):
            ...
            if len(sample) == 0:
                print(f"      ⚠️ Colonne vide, ignorée", file=sys.stderr)
                continue
            ...
            print(f"      Format choisi: {best_name} ({best_success_rate*100:.1f}% succès)", file=sys.stderr)
            ...
            if failed > 0:
                print(f"      ⚠️ {failed} dates invalides → laissées NULL (imputation dans cross_reference)", file=sys.stderr)
            
            if df[col].notna().sum() > 0:
                date_columns[col] = df[col].copy()
                df = df.drop(columns=[col])
            else:
                print(f"      ❌ Échec total, colonne ignorée", file=sys.stderr)
...
def clean_prices(df):
    ...
    for col in df.columns:
        ...
        if any(keyword in col_lower for keyword in price_keywords):
            print(f"   💵 Prix détecté: {col}", file=sys.stderr)
...
def handle_negatives(df, decisions):
    ...
    for col, decision in decisions.items():
        ...
        if llm_says_no_negative or forced_positive:
            ...
            if neg_count > 0:
                reason = "LLM" if llm_says_no_negative else "règle forcée"
                print(f"   ⚠️  {col}: {neg_count} négatifs → valeur absolue ({reason})", file=sys.stderr)
...
class DataCleaner:
    """
    Nettoyeur de données - Sanitize uniquement.
    L'imputation, l'enrichissement et la validation métier
    sont délégués à cross_reference.py
    """
    ...
    def clean(self):
        ...
        print(f"Lignes: {len(self.df)}, Colonnes: {len(self.df.columns)}", file=sys.stderr)
        
        # 1. Nettoyage de base
        self.df = clean_basic(self.df, self.row_threshold, self.col_threshold)
        
        # 2. Créer profil pour LLM
        print("\n📊 Analyse des types de colonnes...", file=sys.stderr)
        ...
        if self.use_llm:
            ...
            if not decisions:
                print("📋 Utilisation règles fallback...", file=sys.stderr)
                decisions = create_fallback_type_decisions(self.df)
        else:
            decisions = create_fallback_type_decisions(self.df)
        
        # 4. Convertir les dates
        print(f"\n🔧 Conversion des types...", file=sys.stderr)
        ...
        for col, date_series in date_columns.items():
            ...
            print(f"   ✅ Date réintégrée: {col}", file=sys.stderr)
        
        # Résumé des NULL restants (pour info, pas d'action)
        null_summary = self.df.isna().sum()
        null_cols = null_summary[null_summary > 0]
        if len(null_cols) > 0:
            print(f"\n📊 NULL restants (à traiter dans cross_reference.py):", file=sys.stderr)
            for col, count in null_cols.items():
                pct = count / len(self.df) * 100
                print(f"   • {col}: {count} ({pct:.1f}%)", file=sys.stderr)
        
        print(f"\n✅ Sanitize finished: {len(self.df)} lines, {len(self.df.columns)} colonnes", file=sys.stderr)
